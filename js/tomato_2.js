...
var sequence = ["pomodoro", "short_break", "pomodoro", "short_break", "pomodoro", "short_break", "pomodoro", "long_break"];
...
function addTrackerModalContent() {
  ...
  if (logLine && logLine.length > 0) {
    ...
    var logLineHTML = logLine.map(function (item) {
      ...
      var inner = "<td>" + item.current + "</td>" + "<td>" + readableStartTime + "</td>" + "<td>" + readableStopTime + "</td>" + "<td><input type='text' class='log-description' value='" + item.description + "' data-id='" + item.id + "'/></td>";

      return "<tr>" + inner + "</tr>";
    });

    var timerLogHTML = "<tr>" + "<th>Session</th>" + "<th>Start time</th>" + "<th>End time</th>" + "<th>Description</th>" + "</tr>" + logLineHTML;
